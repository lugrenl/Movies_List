...
class MovieDetailsFragment : Fragment(R.layout.fragment_movie_details) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Включаем анимацию Shared Element Transition для этого фрагмента.
        // Это говорит системе, что фрагмент готов к анимированному переходу.
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        movie?.let { movieData ->
            // Устанавливаем transitionName для постера
            val posterImageView = view.findViewById<ImageView>(R.id.movie_poster_details)
            posterImageView.setImageResource(movieData.poster)
            // Задаем уникальное имя, чтобы система знала, какой элемент анимировать.
            posterImageView.transitionName = "poster_${movieData.id}"

            view.findViewById<TextView>(R.id.age_limit_details)?.text = movieData.ageLimit
            view.findViewById<TextView>(R.id.movie_title_details)?.text = movieData.title
            view.findViewById<TextView>(R.id.movie_genre_details)?.text = movieData.genre
            view.findViewById<RatingBar>(R.id.movie_rating_details)?.rating = movieData.rating
            view.findViewById<TextView>(R.id.storyline_text_details)?.text = movieData.description
